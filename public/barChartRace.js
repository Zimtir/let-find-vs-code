...
class BarChartRace {
  ...
  resize() {
    ...
    this.fieldWidth = this.hasIcons
      ? this.canvasWidth - ICON_SIZE
      : this.canvasWidth;
    this.fieldHeight = this.showRulers
      ? this.canvasHeight - HEADER_SIZE
      : this.canvasHeight;
  ...
  _getRandomColor(minValue = 0, maxValue = 255) {
    ...
    const invert = (r, g, b) =>
      r + g + b > (maxValue * 3) / 2
        ? `rgb(${minValue},${minValue},${minValue})`
        : `rgb(${maxValue},${maxValue},${maxValue})`;
  ...
  _drawBar(name, value, idx, maxValue, color) {
    const width =
      (value / maxValue) * this.fieldWidth * (1 - RIGHT_FIELD_MARGIN);

    const leadingSiblingValue =
      idx > 0 ? this.sorted[idx - 1].value : value * 2;

    const headerSize = this.showRulers ? HEADER_SIZE : 0;
    const normY = headerSize + idx * (this.barHeight + this.gutter);
    const siblingNormY =
      headerSize + (idx - 1) * (this.barHeight + this.gutter);
    ...
    const positionalY =
      siblingNormY +
      (1 - (diff - THRESHOLD) / (1 - THRESHOLD)) *
        (this.barHeight + this.gutter);
    ...
    this.ctx.fillText(
      name.slice(0, ((width - 20) / this.barFontSize) * 2),
      10,
      y + this.barHeight / 2 + 5
    );
  ...
  _drawDateLabel() {
    ...
    const xOffset = dateLabelFontSize * 10;
